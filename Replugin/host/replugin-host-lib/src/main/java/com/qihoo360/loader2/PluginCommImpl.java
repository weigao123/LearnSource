...
public class PluginCommImpl {
    ...
    public ComponentName loadPluginActivity(Intent intent, String plugin, String activity, int process) {
        ...
        try {
            ...
            // 从缓存中取出Plugin，然后创建Loader放入，并更新Loader中的各种信息
            ...
            if (ai.processName != null) {
                process = -98;//PluginClientHelper.getProcessInt(ai.processName);
            }

            // 选择进程，如果未启动，就先启动进程，再返回目标进程的Binder，PluginProcessPer
            // 一般默认会使用UI进程，从常驻进程拿到了UI进程的Binder，绕了一圈
